...
4. Keep one writer in the dispatched worktree. Execute the entire work package named in the brief; numbered Tasks in its implementation plan are internal steps, not separate dispatches. Keep the receipt `active` across those implement→verify→repair cycles. The current attempt ends on package completion, a blocker, or a decision need; follow step 5 even if the package is unfinished. Never start the next work package.
...
For `needs_decision`, stop all project activity, release with that outcome, then enqueue and report. Do not retain an `active` receipt while waiting for approval or resume a released attempt.
